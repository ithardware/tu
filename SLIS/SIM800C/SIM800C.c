...
#define HTTP_STATUS_ERROR_READ_HTTP_RESPONSE_HEADER 8
...
//init_gprs优化检测功能，将时间延迟缩短
int init_gprs(void)
{
    u8 check_count = 15,status = 0;
    ...
    do {
        status=sim900a_send_data_ack("AT+SAPBR=2,1\r\n", 14, "+SAPBR: 1,1", 100);
        check_count--;
				DEBUG("content:%s\r\n",USART3_RX_BUF);
    } while(status&check_count);
